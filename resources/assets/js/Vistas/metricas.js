...
export default class Metricas extends  React.Component{
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.activeCount !== this.state.selectedCuenta && nextProps.activeCount != null) {
            this.setState({
                ...
                this.cargarMetricas(nextProps.activeCount.label);
            });
        }
    }
    ...
    cargarMetricas(id){
        $.ajax({
            ...
            success: function (data) {
                ...
                this.setState({metricas: metricas},function () {
    ...
    //Funcion para Crear, Modificar, o Eliminar una Metrica
    createUpdateMetricas(){
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                this.cargarMetricas(this.state.selectedCuentaLbl);
            }.bind(this),
            error: function (xhr, status, err) {
                //Error
                console.error(this.props.url, status, err.toString());
    ...
    //Metodo para cargar los datos de una metrica en especifico
    loadMetrica(id){
        $.ajax({
            type: 'GET',
            url: 'metrica/'+id,
        ...
        if(this.state.action== 'POST'){
            ...
            acciones = [
                ...
                <FlatButton
                    ...
                    onTouchTap={this.createUpdateMetricas.bind(this)}
                />
            ];
        } else if(this.state.action== 'PUT'){
            titulo='Modificar Métrica';
            ...
            contenido=[
                <div>
                    <div className="col-md-12">
                        <h2>¿Esta seguro que quiere eliminar la métrica {this.state.nombre}?</h2>
                    </div>
                </div>
            ];
            acciones = [
                ...
                <section className="has-actions">
                    <div className="section-body">
                        <div className = "section-header">
                            <h2 className = "text-primary">Metricas</h2>
                        </div>
                        <BootstrapTable data={this.state.metricas}
                                        pagination={true}
                                        options={{ noDataText :"No hay Métricas para esta cuenta"}}
                                        hover={true}
                                        search={true}>
